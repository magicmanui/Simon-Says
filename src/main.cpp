...
void showPattern(){
  ...
  do
  {
    ...
    string s;
    ostringstream ss;
    ss << currentComplexity-1;
    s = "Level: "+ss.str();
    u8g2.drawStr(8, 16, s.c_str());
  } while (u8g2.nextPage());
...
void onlineSetup(){
  while(digitalRead(buttonLedList[3].button) == LOW){
    Serial.print(8);
  }
  while(1){
    u8g2.firstPage();
    do
    {
      // Put what you want to draw inside this loop.
      u8g2.setFont(u8g2_font_ncenB14_tr);
      u8g2.drawStr(4, 16, "Select blue"); // switch between menu screens using timer and repaint the screen
      u8g2.drawStr(4, 32, "to host");
    } while (u8g2.nextPage());
    int x = millis() + 1500;
    while (x > millis())
    {
    ...
    do
    {
      // Put what you want to draw inside this loop.
      u8g2.setFont(u8g2_font_ncenB14_tr);
      u8g2.drawStr(4, 16, "Select green"); // switch between menu screens using timer and repaint the screen
      u8g2.drawStr(4, 32, "to join");
    } while (u8g2.nextPage());
    x = millis() + 1500;
    while (x > millis())
    {
...
void setup()
{
  ...
  { //!((digitalRead(buttonLedList[0].button) == LOW) || (digitalRead(buttonLedList[1].button) == LOW) || (digitalRead(buttonLedList[2].button)) == LOW || (digitalRead(buttonLedList[3].button) == LOW))){
    ...
    do
    {
      ...
      u8g2.setFont(u8g2_font_ncenB14_tr);
      u8g2.drawStr(4, 16, "Select Game"); // switch between menu screens using timer and repaint the screen
      u8g2.drawStr(4, 32, "green: online");
    } while (u8g2.nextPage());
    x = millis() + 1500;
    while (x > millis())
    {
